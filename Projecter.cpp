...
int main(int argc, char **argv)
{
 ...
 DRect inRect, outRect;                       //unproject and projected exts
 ...
 MathLib::Point newscale, oldscale;
 bool scaleset = false;
 double scale[2] = {0};
 ...
 try
   {
     //parse the arguments
     while (( myopt = getopt(argc, argv, "l:n:p:f:i:?:s")) != -1)
       switch(myopt)
         {
         case 's':
           {
    ...
    inRect.right = inRect.left + oldscale.x * width;
    inRect.bottom = inRect.top - oldscale.y * height;
    
    //create the to projection
    ...
    if (interpolator != 0)
      {
        ...
        pmesh->setSourceMeshBounds(inRect.left, inRect.bottom, 
    ...
    if (pmesh)
      {
        ...
        pmesh->setSourceMeshBounds(outRect.left, outRect.bottom, 
    ...
    //get the new scale
    if (!scaleset)
      newscale = GetConvertedScale(width,height,outRect);
    ...
    out->setSubfileType(0);
    out->setCompression(COMPRESSION_NONE);
    ...
    for (ycounter =0; ycounter < newheight; ycounter++)
      {
        ...
        for (xcounter = 0; xcounter < newwidth; xcounter++)
          {   
            ...
            _x = static_cast<long int>((x - inRect.left) / (oldscale.x) + 0.5);
            _y = static_cast<long int>((inRect.top - y) / (oldscale.y) + 0.5);
